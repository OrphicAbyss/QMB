...
char *PickBotName(int r, char *name) {
	switch (r) {
		case 1: sprintf(name, "%s", "(GloBot) Tomaz");
			break; // God of globot
		case 2: sprintf(name, "%s", "(GloBot) FrikaC");
			break; // God of bots
		case 3: sprintf(name, "%s", "(GloBot) LordHavoc");
			break; // God of engines
		case 4: sprintf(name, "%s", "(GloBot) Dr Labman");
			break; // My favorite deity
		case 5: sprintf(name, "%s", "(GloBot) Necrophilissimo");
			break; // QMB's QC coder (where are you now?)
		case 6: sprintf(name, "%s", "(GloBot) Chrome");
			break; //people of quake
		case 7: sprintf(name, "%s", "(GloBot) Planaria");
			break;
		case 8: sprintf(name, "%s", "(GloBot) KrimZon");
			break;
		case 9: sprintf(name, "%s", "(GloBot) Akuma");
			break;
		case 10: sprintf(name, "%s", "(GloBot) scar3crow");
			break;
		case 11: sprintf(name, "%s", "(GloBot) Quest");
			break; //cookies
		case 12: sprintf(name, "%s", "(GloBot) CocoT");
			break; //boobies
		case 13: sprintf(name, "%s", "(GloBot) KyleMac");
			break;
		case 14: sprintf(name, "%s", "(GloBot) Koolio");
			break;
		case 15: sprintf(name, "%s", "(GloBot) Krust");
			break;
		case 16: sprintf(name, "%s", "(GloBot) BramBo");
			break;
		case 17: sprintf(name, "%s", "(GloBot) Horn");
			break;
		case 18: sprintf(name, "%s", "(GloBot) c0burn");
			break;
		case 19: sprintf(name, "%s", "(GloBot) Heffo");
			break;
		case 20: sprintf(name, "%s", "(GloBot) Randy");
			break;
		case 21: sprintf(name, "%s", "(GloBot) Tei");
			break;
		case 22: sprintf(name, "%s", "(GloBot) Gleeb");
			break;
		case 23: sprintf(name, "%s", "(GloBot) painQuin");
			break;
		case 24: sprintf(name, "%s", "(GloBot) thedaemon");
			break;
		case 25: sprintf(name, "%s", "(GloBot) IceDagger");
			break;
		case 26: sprintf(name, "%s", "(GloBot) MrG");
			break;
		case 27: sprintf(name, "%s", "(GloBot) Electro");
			break;
		case 28: sprintf(name, "%s", "(GloBot) Rick");
			break;
		case 29: sprintf(name, "%s", "(GloBot) Ghostface");
			break;
		case 30: sprintf(name, "%s", "(GloBot) CheapAlert");
			break;
		default:
			sprintf(name, "%s%i", "(GloBot) ", r);
			break;
	}
...
void NextFreeClient(void) {
	...
	// Keep looping as long as there are free client slots
