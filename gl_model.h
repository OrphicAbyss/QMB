...
typedef struct glpoly_s {
	...
	float	verts[4][VERTEXSIZE];	// variable sized (xyz s1t1 s2t2 s3t3 rgb)
	//where xyz is the vertex
	//s1t1 is the normal texture coords
	//s2t2 is the lightmap texture coords
	//s3t3 is the detail/caustics texture coords
...
typedef struct msurface_s {
	...
	struct	msurface_s	*texturechain; //points to a list of surf's with the same texture
	struct	msurface_s	*extra; //chain for detail textures, caustics, etc
...
typedef struct {
	...
	int					gl_texturenumColorMap; //new colour map thing
	int					texels[MAX_SKINS];	// only for player skins
	maliasframedesc_t	frames[1];	// variable sized
} aliashdr_t;

#define	MAXALIASVERTS	2048 //qmb :polycount double 1024
#define	MAXALIASFRAMES	256
#define	MAXALIASTRIS	4096 //qmb :polycount double 2048
