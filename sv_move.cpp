...
/**
 * Returns false if any part of the bottom of the entity is off an edge that is
 * not a staircase.
 */
int c_yes, c_no;
...
realcheck:
	...
	stop[2] = start[2] - 2 * sv_stepheight.getFloat();
	...
	for (x = 0; x <= 1; x++)
		for (y = 0; y <= 1; y++) {
			...
			if (trace.fraction == 1.0 || mid - trace.endpos[2] > sv_stepheight.getFloat())
				return false;
		}
