...
void Sbar_SoloScoreboard (void)
{
	...
	Sbar_DrawString (x+8, 4, str);

	sprintf (str,"Secrets :%3i /%3i", cl.stats[STAT_SECRETS], cl.stats[STAT_TOTALSECRETS]);
	Sbar_DrawString (x+8, 12, str);
...
// time
	...
	Sbar_DrawString (x+184, 4, str);

// draw level name
	l = Q_strlen (cl.levelname);
	Sbar_DrawString (x + 232 - l*4, 12, cl.levelname);
}
	...
	if (hud.value >= 3 || sb_showscores) { //JHL - start
		...
		for (i=0 ; i<7 ; i++)
		{
			if (cl.items & (IT_SHOTGUN<<i) )
			{
			 ...
			 Sbar_DrawPic (vid.width-24, ((vid.height/2)*(-1))+i*16, sb_weapons[flashon][i]);
		...
		for (i=0 ; i<4 ; i++)
		{
			...
			if (num[0] != ' ')
				Sbar_DrawCharacter ( vid.width-56, ((vid.height/2)*(-1))+i*32, 18 + num[0] - '0');
			if (num[1] != ' ')
				Sbar_DrawCharacter ( vid.width-48, ((vid.height/2)*(-1))+i*32, 18 + num[1] - '0');
			if (num[2] != ' ')
				Sbar_DrawCharacter ( vid.width-40, ((vid.height/2)*(-1))+i*32, 18 + num[2] - '0');
		}
	} //JHL - end
		...
		for (i=0 ; i<6 ; i++)
		  if (cl.items & (1<<(17+i)))
		  {
			 ...
			 //MED 01/04/97 changed keys
				if (!hipnotic || (i>1))
				{
				   Sbar_DrawPic (vid.width-96+i*16, -16, sb_items[i]);
				}
			 }
			...
			for (i=0 ; i<4 ; i++)
			{
				if (cl.items & (1<<(28+i)))
				{
					...
					{	// flash frame
						...
						Sbar_DrawPic (vid.width-32 + i*8, -32, sb_sigil[i]);
					if (time &&	time > cl.time - 2)
						sb_updates = 0;
				}
			}
...
void Sbar_Draw (void)
{
   ...
   // armor
		if (cl.items & IT_INVULNERABILITY)
		{
			Sbar_DrawNum (24, -24, 666, 3, 1);
			Sbar_DrawPic (0, -24, draw_disc);
		}
		else
		{
			if (rogue)
			{
				Sbar_DrawNum (24, -24, cl.stats[STAT_ARMOR], 3, cl.stats[STAT_ARMOR] <= 25);
	...
	// health
		Sbar_DrawNum (24, 0, cl.stats[STAT_HEALTH], 3, cl.stats[STAT_HEALTH] <= 25);
	...
	// face
		...
		{
	// ammo count
			Sbar_DrawNum (vid.width-96, 0, cl.stats[STAT_AMMO], 3, cl.stats[STAT_AMMO] <= 10);

	// ammo icon
			if (rogue)
			{
				if (cl.items & RIT_SHELLS)
					Sbar_DrawPic (vid.width-24, 0, sb_ammo[0]);
				else if (cl.items & RIT_NAILS)
					Sbar_DrawPic (vid.width-24, 0, sb_ammo[1]);
				else if (cl.items & RIT_ROCKETS)
					Sbar_DrawPic (vid.width-24, 0, sb_ammo[2]);
				else if (cl.items & RIT_CELLS)
					Sbar_DrawPic (vid.width-24, 0, sb_ammo[3]);
				else if (cl.items & RIT_LAVA_NAILS)
					Sbar_DrawPic (vid.width-24, 0, rsb_ammo[0]);
				else if (cl.items & RIT_PLASMA_AMMO)
					Sbar_DrawPic (vid.width-24, 0, rsb_ammo[1]);
				else if (cl.items & RIT_MULTI_ROCKETS)
					Sbar_DrawPic (vid.width-24, 0, rsb_ammo[2]);
			}
			else
			{
				if (cl.items & IT_SHELLS)
					...
					Sbar_DrawPic (vid.width-24, 0, sb_sbar);
			}
		}
