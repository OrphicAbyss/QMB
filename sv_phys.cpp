...
/**
 * Special move type for Slider mod
 */
void SV_Physics_Snowboard(edict_t *ent) {
	trace_t trace;
	vec3_t move;
	float dist;
	...
	dist = DotProduct(ent->v.velocity, trace.plane.normal) * -1.5;
	VectorMA(ent->v.velocity, dist, trace.plane.normal, ent->v.velocity);
}
...
void SV_Physics_Client(edict_t *ent, int num) {
	...
	switch ((int) ent->v.movetype) {
		...
		case MOVETYPE_FLY:
			//qmb :ladder
		case MOVETYPE_LADDER: //JHL:ADD; Ladder movement fix
			if (!SV_RunThink(ent))
				return;
		...
		case MOVETYPE_SNOWBOARD:
			SV_Physics_Snowboard(ent);
			break;
...
void SV_CheckWaterTransition(edict_t *ent) {
	...
	if (cont <= CONTENTS_WATER) {
		...
		if ((ent->v.watertype != CONTENTS_EMPTY) && (ent->v.watertype != CONTENTS_SOLID)) { // just crossed into water
			SV_StartSound(ent, 0, "misc/h2ohit1.wav", 255, 1);
		}
		ent->v.watertype = CONTENTS_EMPTY;
	...
	for (i = 0; i < sv.num_edicts; i++, ent = NEXT_EDICT(ent)) {
		...
		if (i > 0 && i <= svs.maxclients)
			...
			SV_Physics_Step(ent);
				...
				//qmb :ladder
				|| ent->v.movetype == MOVETYPE_LADDER //JHL:ADD; Ladder movement fix
