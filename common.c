...
//
	...
	{
//		if (strcmp(filename, "progs.dat")&& nomod && strcmp(search->filename, "qmb"))
//			search = search->next;
...
int COM_MultipleSearch (char *searchstring, char **found, int max)
{
	...
	baselen = (int)COM_SkipPath(searchstring) - (int)searchstring;
	...
	for ( ; search; search = search->next)
	{
		...
		// look through all the pak file elements
			...
			for (i=0 ; i<pak->numfiles && numfound < max ; i++)
				if (!Q_strcmp (basestring, pak->files[i].name))
				{       // found a matching mapname.. now to match wildcards.
					...
					while(1)
					{
						...
						case '\0':
							...
							{//Found it!
								found[numfound] = Z_Malloc(strlen(pak->files[i].name)*sizeof(char));
...
next:;
		...
		{               
			...
			do
			{
				...
				found[numfound] = Z_Malloc(strlen(filestring)*sizeof(char));
