...
void Draw_AlphaFill (int x, int y, int w, int h, int c);
extern cvar_t	crosshair;

#define BUTTON_HEIGHT	10
#define BUTTON_START	50
#define	BUTTON_MENU_X	100
#define LAYOUT_RED		75
...
{
	Draw_Pic (x + ((vid.width - 320)>>1), y+(vid.height/2-120), pic);
}
...
// help message
	PrintWhite ((vid.width-(strlen(net_helpMessage[m_net_cursor*2])*8))/2, vid.height-16, net_helpMessage[m_net_cursor*2]);
	PrintWhite ((vid.width-(strlen(net_helpMessage[m_net_cursor*2+1])*8))/2, vid.height-8, net_helpMessage[m_net_cursor*2+1]);
}
...
void M_Keys_Draw (void)
{
	...
	if (bind_grab)
		PrintWhite ((vid.width-(37*8))/2, vid.height-8, "Press a key or button for this action");
	else
		PrintWhite ((vid.width-(35*8))/2, vid.height-8, "Enter to change, backspace to clear");
...
#define HELP_INTRO		0
#define HELP_CVARS_A	1
#define HELP_CVARS_B	2
#define HELP_ID			3
...
void M_Help_Draw (void)
{
	...
	PrintWhite (vid.width/2 - (strlen(onscreen_help)*4), vid.height-16, onscreen_help);
	PrintWhite (vid.width/2 - 156, vid.height-8, "Use the arrow keys to scroll the pages.");

	if (help_page == HELP_INTRO)
	{
		M_PrintWhite (16, 148, "This program uses Quake-technology;\n");
		M_PrintWhite (16, 156, "Quake is a trademark of Id Software,\n");
		M_PrintWhite (16, 164, "inc., (c)1996 Id Software, inc. All\n");
		M_PrintWhite (16, 172, "rights reserved. Technology used\n");
		M_PrintWhite (16, 180, "under GNU General Public Licence.\n");
	}
	else if (help_page == HELP_ID)
	{
		M_PrintWhite (16, 12,  "        QUAKE by id Software\n\n");
	...
	else if (help_page == HELP_CVARS_A)
	{
		M_PrintWhite (16, 12,  "   New Cvars and console commands:\n\n");
	}
	else if (help_page == HELP_CVARS_B)
	{
	}
}
...
// help message
	...
	{
		x = (vid.width/2) - (42*4);
		PrintWhite (x, vid.height-16, "More than 4 players requires using command");
		x = (vid.width/2) - (41*4);
		PrintWhite (x, vid.height-8, "line parameters; please see techinfo.txt.");
	}
}
