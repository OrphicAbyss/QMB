...
int VCR_Init(void) {
	...
	SystemFileManager::FileRead(vcrFile, &next, sizeof (next));
	return 0;
}

void VCR_ReadNext(void) {
	if (SystemFileManager::FileRead(vcrFile, &next, sizeof (next)) == 0) {
		next.op = 255;
...
int VCR_GetMessage(qsocket_t *sock) {
	...
	SystemFileManager::FileRead(vcrFile, &ret, sizeof (int));
	if (ret != 1) {
		VCR_ReadNext();
		return ret;
	}

	SystemFileManager::FileRead(vcrFile, &net_message.cursize, sizeof (int));
	SystemFileManager::FileRead(vcrFile, net_message.data, net_message.cursize);
...
qsocket_t *VCR_CheckNewConnections(void) {
	...
	*(long *) (&sock->driverdata) = next.session;
	...
	SystemFileManager::FileRead(vcrFile, sock->address, NET_NAMELEN);
	VCR_ReadNext();
